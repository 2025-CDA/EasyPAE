...
function SearchBar({ className }) {
    ...
    const handleSearch = () => {
        const results = datas.filter(
            (item) => item.name.toLowerCase().includes(search.toLowerCase()) //Filtre les données pour ne garder que celles dont le name correspond à la recherche
        );
            ...
            <div className="flex">
                <div className="relative flex-grow">
                    <Input
                        ...
                        className="py-3 ps-5 pe-5 w-full border border-gray-200 rounded-l-lg rounded-r-none sm:text-sm focus:border-1"
                        value={search}
